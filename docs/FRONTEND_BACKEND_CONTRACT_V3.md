...
- 원천이 공통 절대 언급량을 제공하지 않으므로 화면은 “순위 반응형 관심 흐름”으로 표시합니다. 각 실제 관측점은 X·Google 원천 순위 45%, 원천·이전 공개 순위 변화 20%, 관측 지속성 15%, 현재 공개 순위 20%를 조합한 0~100 표시지수이며 절대 게시물 수로 해석하지 않습니다. 공개 순위는 표시지수와 선 굵기·점 크기·모션 속도에 반영되지만, 원천 `series`·Python 점수와 순위·관측 시각·결측 구간·기업 시장데이터는 바꾸지 않습니다.
...
- 공식 버전은 `spread35_velocity25_breadth20_persistence10_recency10_v3`임
