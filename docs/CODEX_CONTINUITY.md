...
- 운영 감사 `failures`가 비어 있고, 최근 24시간의 실제 관측시간 수·결측시간·마지막 성공시각이 사실대로 기록돼 있습니다. 결측은 허용하되 이전값 재사용·보간·생성으로 채우지 않습니다.
- 프런트 `presentation_feed`는 v4 실제 관측 계약이고 완성 카드 0~10개만 포함합니다. 원격 배포 영수증은 매일 06:00 KST 공개본에 한 번 요구합니다.

## 8. 현재 허용된 외부 보조 데이터

- NAVER 뉴스: 국내 촉발 맥락 근거. 인증·런타임 정책 실패는 비차단 상태로 기록하고 순위에는 반영하지 않습니다.
- 공식 기업·행사 페이지: 기업 관계와 촉발 맥락 근거. 공개 URL과 검수상태를 함께 보존합니다.
- 시장 참고정보: 공급자·기준시각·공개 source URL이 모두 있는 실제 관측값만 화면에 표시합니다.
- YouTube·Instagram·NAVER 블로그·카페·검색트렌드는 활성 경로에서 제외합니다.
...
- 기업 10개와 역할 카테고리 2~4개를 증명하지 못한 트렌드는 유지되며 기업 카드만 보강 대기 상태입니다. 이는 억지 연결을 막는 품질 게이트입니다.
