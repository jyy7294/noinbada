...
- 내부 선별점수와 원천 순위는 X·Google만 동일 비중으로 사용합니다. NAVER 뉴스는 촉발 맥락과 근거 URL을 보강하지만 점수·그룹·순위를 바꾸지 않습니다.
- YouTube·Instagram·NAVER 블로그·검색트렌드는 현재 활성 홈 수집·분석에서 제외합니다.
...
- 기업은 완결된 증거 경로가 있는 서로 다른 국내외 상장기업 10개 이상이고 역할 카테고리가 3~4개면 준비 완료입니다. 부족하면 트렌드는 유지하고 `enrichment_pending`으로 두며 임의로 채우지 마십시오.
- pykrx(국내)와 Yahoo Finance(해외)는 출처 URL이 있는 실제 일별 시장 참고자료이며 관계·상승 예측 근거가 아닙니다. 다만 공개 v4 기업의 현재 상장 여부는 발행일 기준 거래소 전체 종목 명부 또는 공식 현재 종목 등록부로 별도 검증해야 합니다. Yahoo 가격 이력만 있는 해외 종목은 현재 상장 증명이 아니므로 공개에서 fail-closed 처리합니다.
...
- 프런트 공개 카드마다 근거 온톨로지가 완결되고 발행일 기준 공식 현재 상장 증명이 있는 서로 다른 국내외 상장기업이 정확히 10개이고 역할 카테고리가 3~4개인지
- `hourly-source-proof-v3`와 현재 frontend result-quality 사전검사를 원격 게시 전에 통과하는지
- 최근 24시간의 실제 관측·누락 시각이 감사 정보에 정확히 기록되고, 이전값 재사용이나 합성 채움 없이 불변 영수증과 원격 해시가 일치하는지
