...
- 기업은 완결된 증거 경로가 있는 서로 다른 국내외 상장기업 6개 이상이면 준비 완료입니다. 부족하면 트렌드는 유지하고 `enrichment_pending`으로 두며 임의로 채우지 마십시오.
...
- 프런트 Top10마다 근거 온톨로지가 완결된 서로 다른 국내외 상장기업이 최소 6개인지
- `hourly-source-proof-v2`와 `frontend-result-quality-v2` 사전검사를 원격 게시 전에 통과하는지
