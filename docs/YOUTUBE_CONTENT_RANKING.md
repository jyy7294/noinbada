# YouTube 대한민국 콘텐츠 트렌드 실험 기록 (현재 비활성)

이 문서는 과거 실험을 보존하기 위한 아카이브입니다. 현재 운영 계약에서는 YouTube를 순위·선발·맥락 보강에 사용하지 않습니다. X와 Google 실측만 순위 입력이고, NAVER 뉴스와 공식 페이지는 맥락·기업 근거에만 사용합니다.
