...
기능 단위 작업을 마칠 때마다 다음 순서를 지킵니다. 장시간 작업은 독립적으로 검증 가능한 단위가 생길 때 또는 늦어도 30~45분마다 체크포인트하며, 종료·컨텍스트 소진 전에는 반드시 원격 상태를 확인합니다. 시간만 지났다는 이유로 깨진 코드를 자동 커밋하지 않습니다.
...
4. 명시한 작업 파일만 스테이징합니다. 이미 스테이징된 무관 파일이나 범위 밖 dirty 파일이 있으면 중단합니다.
5. 검증을 통과한 변경만 명확한 커밋으로 만들고, 테스트 시작과 push 직전의 `origin/main` SHA가 같음을 확인합니다.
6. 팀 변경이 있으면 덮어쓰지 말고 통합·재검증합니다. 최종 검증 커밋은 반드시 `main`에 병합하고 원격 SHA 일치를 확인합니다.
7. 안정 실행 checkout을 `origin/main`으로 fast-forward하고 의존성을 갱신합니다.
8. 실제 파이프라인을 한 번 실행해 SQLite, publication, `live-data` 원격 게시까지 확인합니다.
9. 문서의 현재 상태와 남은 수동 게이트를 갱신합니다.
...
새 작업은 먼저 `CURRENT_STATE.json`을 읽습니다. 검증된 코드 체크포인트는 `scripts/checkpoint-main.ps1`로만 게시하고, 실행 checkout 갱신은 `scripts/promote-runtime.ps1`의 안전 검사를 통과해야 합니다.
