...
대표어·사건 그룹 → 전체 순위 → 약한 표시 필터
          ↓
관련어 0~5 → 증거 온톨로지 → 국내외 상장기업 3개 준비 게이트
          ↓
...
- `company_ready_trends`: 증거 기반 상장기업 3개 이상 준비된 별도 목록
...
- 검수된 기업이 3개 미만이어도 Top10에서는 제거하지 않습니다. 기업 카드만 보류하고 보강 큐에 남깁니다.
...
모든 edge에 URL·근거 유형·기준일·검수상태가 있어야 합니다. 완결 경로를 가진 서로 다른 국내외 상장기업이 3개 이상일 때 `companies`가 준비 완료입니다. 0~2개면 트렌드는 유지하되 기업 카드는 `enrichment_pending`으로 표시합니다. 기업을 채우기 위한 무근거 단어 연상은 금지합니다.

3개에 못 미친 메인 트렌드는 `ontology_enrichment_queue`에 부족한 경로 수와 실제 관측 표현을 남깁니다. 이후 공식 기업자료·공시·검수된 기사·산업구조 근거를 추가하는 연구 대상이며, 큐 자체는 순위에 영향을 주지 않습니다.
...
src/trzip/ontology.py                    증거 그래프와 3개 준비 게이트
...
- 온톨로지 3개 미달 트렌드는 유지되며 증거를 추가하기 전까지 기업 카드가 보강 대기 상태입니다.
