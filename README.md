...
제품 정의, 현재 구현과 목표 API 아키텍처는 [TRZIP 제품·기술·발표 기준안](docs/TRZIP_PRODUCT_TECHNICAL_BLUEPRINT.md)을 기준으로 합니다. 발표의 전체 순서·기술 파트·Q&A 기준은 [TRZIP 발표 마스터 플랜](docs/TRZIP_PRESENTATION_MASTER_PLAN.md)을 따릅니다. 현재 쇼케이스 UI와 실제 운영 publication을 혼동하지 않습니다.
...
- 자동 선발 순위는 기업 수와 독립적으로 유지합니다. 다만 프런트 공개 카드는 `main` 레인, URL 근거가 있는 `why_now`, 관련 키워드 정확히 5개, 검수된 현재 상장기업 정확히 10개, 역할 카테고리 3~4개, 기업과 연결되는 서로 다른 키워드 2개 이상, 공개 기업 10곳 각각의 키워드 연결과 URL 근거를 모두 충족해야 합니다.
- 단독 일반명사·산업명·기업명은 원장과 전체 순위에는 남지만 현재 사건 수식어가 없으면 `review`입니다. 스포츠 대진은 한국어·영어 변형을 같은 사건으로 병합하고 표기는 `팀A vs 팀B`로 고정하되, 단순 대진·스코어 조회는 `review`로 분리하고 직관·예매·결승·우승처럼 별도의 참여·관람·사건 신호가 있을 때만 `main` 후보로 올립니다.
- 일일 원격 공개 전 `latest/final-publication-review.json`을 생성합니다. 제품 소유자가 그 검토표의 정확한 SHA-256에 대해 승인한 event key만 `presentation_feed`에 들어가며, 승인 영수증이 없거나 변조·시각 불일치가 있으면 원격 게시 전에 중단합니다.

최종 승인 절차는 다음과 같습니다. 먼저 `final-publication-review.json`의 자동 통과 후보와 모든 탈락 사유를 찬희님께 보여드리고, **그 검토표에 대한 현재 대화의 명시적 승인**을 받은 event key만 아래 도구로 영수증화합니다. 이전 시각의 승인이나 포괄적인 “알아서 게시” 발언을 재사용하지 않습니다.

```powershell
python scripts/approve-final-publication.py `
  --review <publication-root>\latest\final-publication-review.json `
  --approval-root <runtime-root>\final-publication-approvals `
  --approved-by "이찬희" `
  --event-key <승인할-event-key>
```

0개 공개를 명시적으로 승인할 때는 `--event-key`를 생략합니다. 자동 필터에서 탈락한 event key는 승인 도구 자체가 거부합니다.
...
모든 edge에 URL·근거 유형·기준일·검수상태가 있어야 합니다. 완결 경로를 가진 서로 다른 국내외 현재 상장기업이 10개 이상이고 역할 카테고리가 3~4개이며 공개 기업마다 최소 하나의 공개 키워드 연결이 있을 때 `companies`가 준비 완료입니다. 0~9개이거나 연결 근거가 비어 있으면 검증 후보는 보존하되 기업 카드는 `enrichment_pending`으로 표시합니다. 기업을 채우기 위한 무근거 단어 연상은 금지합니다.
...
운영 감사 결과는 `PASS`, `PROVISIONAL`, `FAIL` 중 하나입니다. 발표·인수인계 전에는 X·Google의 마지막 성공시각, 최근 24시간 실제 관측시간 수·결측시간, 프런트 v4 계약 성공, 해당 일일 공개본의 원격 영수증을 함께 확인합니다. 최근 24시간의 다른 결측 시각은 허용하되 재사용·보간·생성으로 메우지 않습니다. 매일 06:00 KST 원격 게시에는 최근 24시간 안에 X·Google이 각각 최소 한 번 완전하게 관측됐고, 06:00 정각에도 X 30개와 Google 전체 목록이 모두 정상 관측돼야 합니다.
