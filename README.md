...
기간별 점수 = 기간강도 40%
             ...
             + 원천별 반복 관측 지속성 20%
             + 마지막 관측 신선도 15%
             + 기간 내 X·Google 교차관측 5%
```

- `ranking_views`: 일간 24시간·주간 168시간·월간 720시간에 실제 관측된 전체 후보
- `unified_ranking`: 기본 주간 화면의 호환 별칭, 제한 없음
...
기간강도는 원천별 정규화 위치의 `70% 신선도 가중 평균 + 30% 기간 최고점`입니다. 모멘텀은 직전 동일 기간을 우선 비교하고, 불가능하면 현 기간 전반부와 후반부를 비교하며, 둘 다 없으면 중립점수를 적용합니다. 원시 검색량·게시량을 플랫폼 간 합산하지 않으며 카테고리·기사·기업 수는 점수에 영향을 주지 않습니다. 60일 이력은 생애주기 판별에만 쓰고 점수에는 넣지 않습니다.
